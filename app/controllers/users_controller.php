 ...
 */
	...
	public function reset($username = null, $key = null) {
		...
		if (!empty($this->data) && isset($this->data['User']['password'])) {
			...
			if ($this->User->save($user['User'], $options)) {
				$this->Session->setFlash(__('Wachtwoord is gerest.', true), 'default', array('class' => 'success'));
				$this->redirect(array('action' => 'login'));
			} else {
				$this->Session->setFlash(__('Er is een fout opgetreden. Probeer het later nog eens.', true), 'default', array('class' => 'error'));
			}
		}
	...
	public function logout() {
		$this->Session->setFlash(__('U bent succsvol afgemeld.', true), 'default', array('class' => 'success'));
		$this->redirect($this->Auth->logout());
	}
